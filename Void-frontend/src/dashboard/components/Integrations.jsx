...
import {
  ...
  FaCopy, FaCheckCircle, FaSpinner, FaUnlink, FaSave, FaTelegram, FaInstagram
} from "react-icons/fa";
...
function TechAiModalPreview({ isOpen, onClose, config }) {
  ...
  const brandColor = config?.brandColor || "#16a085";
  ...
  return (
    <div className="fixed inset-0 z-[9999]">
      ...
      />
      {/* panel */}
      <div className="absolute bottom-4 right-4 sm:bottom-6 sm:right-6 w-full max-w-lg">
        <div
          className="relative bg-zinc-900 rounded-2xl border border-zinc-800 flex flex-col"
          style={{ maxHeight: 700, height: "calc(100vh - 4rem)" }}
        >
    ...
    <div className="bg-zinc-900 rounded-xl shadow-lg p-6 md:p-8 border border-zinc-800">
      ...
      <Tab.Group>
            ...
            <Tab
              ...
              className={({ selected }) =>
                classNames(
                  ...
                  selected ? "bg-[#16a085] text-white shadow" : "text-gray-300 hover:bg-zinc-700 hover:text-white"
                )
              }
            >
        ...
        <Tab.Panels className="mt-2">
          ...
          <Tab.Panel><InstagramPanel /></Tab.Panel>
          <Tab.Panel><TelegramPanel /></Tab.Panel>
        </Tab.Panels>
      </Tab.Group>
    </div>
  );
...
function WebsitePanel() {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      if (!token) return;
      try {
        const res = await fetch("/api/widget/config", { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleDomainsChange = (e) =>
    setConfig({ ...config, allowedDomains: e.target.value.split("\n").map((d) => d.trim()).filter(Boolean) });
  const handleHandoffChange = (e) =>
    setConfig({ ...config, handoffTriggerKeywords: e.target.value.split("\n").map((k) => k.trim().toLowerCase()).filter(Boolean) });
  const handleLanguageChange = (e, langCode) => {
    const { name, value } = e.target;
    setConfig((prev) => ({
      ...prev,
      languages: { ...prev.languages, [langCode]: { ...prev.languages[langCode], [name]: value } },
    }));
  };
...
import { TechAiModal } from '@your-company/react-widget';
...
export const ChatTrigger = () => {
  ...
  const currentUserId = 'user-session-abc-123';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const phpCode = `<?php
$organizationId = get_user_organization_id();
$apiBaseUrl = "${API_BASE_URL}";
?>
<div id="tech-ai-widget-container"></div>
<script>
// include TechAiWidget class here...
document.addEventListener('DOMContentLoaded', () => {
  const widget = new TechAiWidget({
        ...
        <Tab.Group>
          ...
          <Tab.Panels className="mt-2">
            <Tab.Panel className="space-y-6 rounded-xl py-3">
              ...
              <CodeSnippetDisplay code={"npm install @your-company/react-widget"} />
              ...
              <InstructionStep number="3" title="Implement the Component">
                Use the hook and pass the configuration to the TechAiModal component.
              </InstructionStep>
            ...
            <Tab.Panel className="space-y-6 rounded-xl py-3">
              ...
              <InstructionStep number="2" title="Include the Script">
                Include the script on your page before the closing body tag.
              </InstructionStep>
            </Tab.Panel>

            <Tab.Panel className="space-y-6 rounded-xl py-3">
              <InstructionStep number="1" title="Implement the Client-Side Logic">
                Follow the HTML/JS tab to create the TechAiWidget class.
              </InstructionStep>
              <InstructionStep number="2" title="Embed in Your PHP Template">
                Use echo to securely inject the organizationId from your server-side code.
              </InstructionStep>
...
function WhatsAppPanel() {
  ...
  useEffect(() => {
    ...
    const handleStatusUpdate = ({ status: newStatus, message }) => {
      setIsLoading(false);
      setStatus(newStatus);
    ...
    socket.on("whatsapp-log", handleLogUpdate);
    ...
    return () => {
      ...
      socket.off("whatsapp-log", handleLogUpdate);
    };
  }, [socket, token]);
  ...
  const handleStop = () => {
    if (window.confirm("Are you sure? This will log out and require a new QR scan on restart.")) {
      handleAction("stop");
    }
  };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <div>
        <div className="flex items-center gap-3 mb-4">
          <div
            className={`w-3 h-3 rounded-full ${statusColor[status]?.replace("text-", "bg-")} ${status === "connected" ? "animate-pulse" : ""
              }`}
          />
        ...
        <div className="flex gap-4 mb-8">
          <button
            ...
            disabled={isLoading || status === "connected" || status === "connecting"}
            className="flex items-center gap-2 px-5 py-2.5 bg-green-600 text-white rounded-lg font-bold hover:bg-green-700 disabled:bg-zinc-700 disabled:cursor-not-allowed"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-red-600 text-white rounded-lg font-bold hover:bg-red-700 disabled:bg-zinc-700 disabled:cursor-not-allowed"
          >
        ...
        <div className="w-full h-64 flex items-center justify-center bg-zinc-950 rounded-lg p-4 border border-zinc-800">
          ...
          {!isLoading && status === "connected" && (
            <div className="text-center">
              ...
              <p className="text-gray-300 font-semibold">Service is connected and running.</p>
            </div>
          )}
          {!isLoading && status === "disconnected" && (
            <p className="text-gray-500">Service is stopped. Press "Start" to begin.</p>
          )}
        </div>
      </div>
...
function FacebookPanel() {
  ...
  const loadIntegrationStatus = useCallback(async () => {
    if (!token) return;
    try {
      const res = await fetch("/api/facebook/config", { headers: { Authorization: `Bearer ${token}` } });
      if (!res.ok) throw new Error(`Network response was not ok: ${res.statusText}`);
      const data = await res.json();
      setConfig((prev) => ({ ...prev, callbackUrl: data.callbackUrl, verifyToken: data.verifyToken }));
      setStatus(data.status || "inactive");
    } catch (error) {
      setFeedback({ message: `Configuration Error: ${error.message}`, type: "error" });
      setStatus("inactive");
    }
  }, [token]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch("/api/facebook/activate", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ pageAccessToken: config.pageAccessToken, appSecret: config.appSecret }),
      });
      const data = await res.json();
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div className="p-6 bg-zinc-950 rounded-lg border border-zinc-800 space-y-6">
        <InstructionStep number="1" title="Set Up Your Webhook">
          In your App's Messenger Settings, click Add Callback URL. Paste the URL and Verify Token from below.
        </InstructionStep>
        <div>
      ...
      <div className="p-6 bg-zinc-950 rounded-lg border border-zinc-800 space-y-6">
        <InstructionStep number="2" title="Enter Your Page Credentials">
          Generate a Page Access Token and find your App Secret in Basic Settings.
        </InstructionStep>
        <div>
...
/* ---------------- Simple placeholders so tabs compile ---------------- */
function InstagramPanel() {
  return (
    <div className="p-6 bg-zinc-950 rounded-lg border border-zinc-800 text-gray-300">
