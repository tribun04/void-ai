...
export function TechAiModal({ isOpen, onClose, currentUserId, lang = 'en' }) {
  ...
  const langButtonClass = (langCode) =>
    `px-2 py-1 text-xs rounded-md transition-colors font-medium ${currentLang === langCode
        ...
        <div className="flex-grow overflow-y-auto p-4 space-y-4">
          ...
          {messages.map((message, index) => (
            <div key={index} className={`flex items-end gap-2 ${message.type === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`w-fit max-w-[85%] px-4 py-3 rounded-2xl ${message.type === "user" ? "bg-indigo-800 text-white rounded-br-lg" :
        ...
        <div className="p-4 border-t border-[#2A2F3B] flex-shrink-0 relative">
          ...
          <div className="absolute right-6 top-1/2 -translate-y-1/2 flex items-center">
            <button
              ...
              className={`p-2 rounded-full transition-all duration-300 ${!isInputDisabled && inputText.trim() ? "bg-indigo-800 text-white" : "bg-transparent text-[#6B7280] cursor-not-allowed"
                }`}
            >
