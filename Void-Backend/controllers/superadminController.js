...
exports.getAllUsers = async (req, res) => {
  ...
  try {
    let sql =
      "SELECT id, tenantId, fullName, email, companyName, role, planName, paymentStatus, apiKey, createdAt FROM users WHERE role != 'superadmin'";
    const params = [];
...
exports.createUser = async (req, res) => {
  ...
  try {
    ...
    const userId = uuidv4();
    await db.query(
      "INSERT INTO users (id, tenantId, fullName, companyName, email, passwordHash, role) VALUES (?, ?, ?, ?, ?, ?, ?)",
      [
        userId,
        tenantId,
        fullName,
...
exports.activateUser = async (req, res) => {
  ...
  try {
    ...
    console.log(
      `✅ User ${id} activated successfully. API Key has been generated.`
    );
    res
      .status(200)
      .json({ message: "User activated successfully!", apiKey: apiKey });
  } catch (error) {
...
exports.activateTenant = async (req, res) => {
  ...
  try {
    ...
    if (!adminUser) {
      ...
      return res
        .status(404)
        .json({ message: "Admin user for this tenant not found." });
    }
    ...
    console.log(
      `✅ Tenant ${tenantId} activated with a limit of ${tokenLimit} tokens.`
    );
    res
      .status(200)
      .json({ message: "Tenant activated successfully!", apiKey: apiKey });
  } catch (error) {
...
exports.getTenantById = async (req, res) => {
  ...
  try {
    ...
    if (!companyAdmin) {
      return res
        .status(404)
        .json({ message: "No admin user found for this tenant." });
    }
    ...
    res
      .status(500)
      .json({ message: "Server error while fetching tenant details." });
  }
};
...
exports.trainAIEntry = async (req, res) => {
  ...
  if (!tenantId || !intent || !baseResponse) {
    return res
      .status(400)
      .json({ message: "tenantId, intent, and baseResponse are required." });
  }
  try {
    ...
    res
      .status(201)
      .json({ message: `Successfully trained intent '${formattedIntent}'.` });
  } catch (error) {
...
exports.getAIEntries = async (req, res) => {
  ...
  if (!tenantId) {
    return res
      .status(400)
      .json({ message: "A tenantId query parameter is required." });
  }
  try {
    const [entries] = await db.query(
      "SELECT id, intent, response_en FROM ai_entries WHERE tenantId = ?",
      [tenantId]
    );
...
exports.deleteAIEntry = async (req, res) => {
  ...
  if (!intent || !tenantId) {
    return res.status(400).json({
      message: "Intent and tenantId are required to delete an entry.",
    });
  }
  try {
    await db.query("DELETE FROM ai_entries WHERE intent = ? AND tenantId = ?", [
      intent,
      tenantId,
    ]);
...
exports.getTodaysConversationsCount = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to get today's conversation count." });
  }
};
