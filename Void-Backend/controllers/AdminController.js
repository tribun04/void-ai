...
exports.createAgent = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Agent created successfully", user: newUser });
  } catch (err) {
  ...
  try {
    ...
    if (!tenant) {
      return res.status(404).json({
        message: "API details not found. Your account may not be active yet.",
      });
    }
    ...
    res
      .status(500)
      .json({ message: "Server error while fetching your API details." });
  }
};
...
exports.activateTenant = async (req, res) => {
  ...
  try {
    ...
    const [tenantRows] = await connection.execute(
      "SELECT status, planId FROM tenants WHERE id = ?",
      [tenantId]
    );
    ...
    if (tenantRows[0].status === "active") {
      ...
      return res
        .status(409)
        .json({ message: "Conflict: This tenant is already active." });
    }

    const planId = tenantRows[0].planId;
    const [planRows] = await connection.execute(
      "SELECT tokenLimit FROM plans WHERE id = ?",
      [planId]
    );
    ...
    await connection.execute(
      "UPDATE invoices SET status = 'paid' WHERE tenantId = ? AND status = 'pending'",
      [tenantId]
    );
    ...
    res
      .status(200)
      .json({ message: `Tenant ${tenantId} has been successfully activated.` });
  } catch (error) {
...
exports.updateTenantTokens = async (req, res) => {
  ...
  if (totalAllocated === undefined || used === undefined) {
    return res
      .status(400)
      .json({ message: "Both totalAllocated and used values are required." });
  }
  try {
    const sql = `
            INSERT INTO tenant_tokens (tenantId, totalAllocated, used) 
    ...
    res
      .status(200)
      .json({ message: `Tokens for tenant ${tenantId} updated successfully.` });
  } catch (error) {
...
exports.getAgentsForMyTenant = async (req, res) => {
  ...
  if (!tenantId) {
    return res.status(403).json({
      message: "Could not identify your organization from your login token.",
    });
  }
...
exports.deleteAgent = async (req, res) => {
  ...
  try {
    ...
    if (result.affectedRows === 0) {
      return res.status(404).json({
        message:
          "Agent not found or you do not have permission to delete them.",
      });
    }
    res.status(200).json({ message: "Agent deleted successfully." });
  } catch (error) {
