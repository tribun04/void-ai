    ...
    async getConfig(organizationId) {
        ...
        try {
            ...
            const defaultConfig = {
                ...
                handoffTriggerKeywords: ['agent', 'human', 'speak to a person', 'live chat'],
                languages: {
                    en: {
                        welcomeMessage: "Hello! How can I help you today?",
...
router.put('/config', async (req, res) => {
    ...
    try {
        ...
        const newConfig = { ...currentConfig, ...req.body, organizationId: currentConfig.organizationId };
...
router.get('/public-config/:organizationId', async (req, res) => {
    ...
    try {
        ...
        const isAllowed = fullConfig.allowedDomains.some(domain => requestOrigin?.includes(domain));
        ...
        if (!isAllowed && !isDev && fullConfig.allowedDomains.length > 0) {
            console.warn(`[Widget Security] Blocked request from unauthorized origin: ${requestOrigin} for org: ${organizationId}`);
            return res.status(403).json({ error: 'This domain is not authorized to use this widget.' });
        }
        ...
        console.error(`Error loading public config for org ${organizationId}:`, error);
