...
app.get("/api", (req, res) =>
  res.status(200).json({ message: "VOID AI Support Backend is running ✅" })
);

const httpServer = http.createServer(app);

// ✅ UNIFIED SOCKET.IO SERVER SETUP
const io = new Server(httpServer, {
  ...
  // ✅ THE FIX: Add this line to allow connections from older Socket.IO clients.
...
voipWss.on("connection", (ws, request) => {
  ...
  console.log(
    `🔗 VOIP WebSocket connection established for provider: ${providerName}!`
  );
...
io.on("connection", (socket) => {
  ...
  if (socket.user && socket.user.role === "superadmin") {
    ...
    console.log(
      `👑 Superadmin ${socket.user.name} joined the private superadmin room.`
    );
  }
  ...
  socket.on("get-ai-reply", async ({ userInput, language }, callback) => {
    try {
      ...
      callback({
        reply:
          "Më falni, pata një problem me sistemin e inteligjencës artificiale. Ju lutem provoni përsëri.",
      });
    }
  });
  ...
  socket.on("agent-reconnected", ({ conversationIds }) => {
    ...
    for (const userId of conversationIds) {
      ...
      console.log(
        `🔄 [RECONNECT] Agent ${socket.user.name} re-linked to conversation ${userId}`
      );
    }
  });
    ...
    async ({ userId, agentSocketId, initialMessage }) => {
      ...
      if (userId.includes("@c.us")) {
        ...
        await sendFacebookMessage(
          userId,
          `Ju jeni lidhur me agjentin tonë: ${socket.user.name}.`
        );
      } else {
  ...
  socket.on("end-chat", async ({ userId }) => {
    ...
    console.log(
      `🚫 Chat ended by agent ${socket.user.name} for user ${userId}`
    );
  ...
  socket.on("whatsapp_ready", () => {
    ...
    if (tenantId) {
      ...
      console.log(
        `[Socket] ✅ Relayed 'ready' signal to room: tenant-room-${tenantId}`
      );
    }
  });
  socket.on("qr_code", (data) => {
    ...
    if (tenantId && data.qr) {
      ...
      console.log(
        `[Socket] ✅ Relayed QR code to room: tenant-room-${tenantId}`
      );
    }
  });
